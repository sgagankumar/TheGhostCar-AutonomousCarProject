...
**Driving Prediction/**  
	...
	For Training Procedures and Techniques refer to my other repository - [ML-Ghost-Car-AutonomousDriving](https://github.com/sgagankumar/ML-Ghost-Car-AutonomousDriving)
...
5. **Establish TCP Communication:** Create a TCP Server connection to send back valuable data to the prototype model. Run `python uploadTCP.py` to start a server to send data upon request from the RPi.

6. **Self-driving of prototype**: Run `python3 GhostCarDrive.py` on RPi4, wait for the program to perform GPIO pin setup and establish a connection with the TCP server. Once the above actions are performed the car with start self-driving.
