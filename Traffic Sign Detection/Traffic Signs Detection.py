...
"""main.py: Recognises and Classifies 43 Traffic Signs, Using CNN on Tensorflow backend."""
...
def prepro(img,model1):
	#Preprocess image to 32x32 for prediction
	img = np.asarray(img)
	...
	#Print the predicted Traffic Sign to Debug
	# print('Result :',name[resultid])
...
def findLargestSign(image, contours, threshold, distance_theshold, model1):
	...
	for c in contours:
		...
		if is_sign and distance > max_distance and distance > distance_theshold:
			...
			sign = cropSign(image,coordinate,model1)
...
def localization(image, min_size_components, similitary_contour_with_circle, count, current_sign_type, model1):
	...
	sign, coordinate = findLargestSign(original_image, contours, similitary_contour_with_circle, 15, model1)
...
def main():
	...
	vidcap = cv2.VideoCapture(0) # Specify the Video Source
	...
	while True:
		...
		coordinate, image, sign_type, text = localization(frame, 1, 10, count, current_sign, model1)
